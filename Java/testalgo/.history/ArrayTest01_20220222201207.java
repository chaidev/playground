...
class ArrayTest01 {
    public static void main(String[] args) {
        int[] inta = new int[] { -2, 3, 1, 1, 4, 2, 6, 5, 7, 8 };
    ...
    static int findMaxIntV3(int[] A) {
        ...
        Integer[] myArraySet = Arrays.stream(A).boxed().sorted().filter(i -> i > 0).toArray(Integer[]::new);

        for (Integer a : myArraySet) {
            int index = Arrays.binarySearch(myArraySet, a);
            if (index > 0) {
                result++;
            }
            else {
